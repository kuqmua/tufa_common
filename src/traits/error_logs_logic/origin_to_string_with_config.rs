...
pub trait OriginToStringWithConfigLifetimeWithSerializeDeserialize<'a, ConfigGeneric> {
    fn origin_to_string_with_config_lifetime_with_serialize_deserialize(
        &self,
        config: &ConfigGeneric,
    ) -> String;
}

impl<'a, SelfGeneric, ConfigGeneric> OriginToStringWithConfigLifetimeWithSerializeDeserialize<'a, ConfigGeneric> for SelfGeneric
where
    SelfGeneric:
        crate::traits::error_logs_logic::source_to_string_without_config::SourceToStringWithoutConfigLifetime<'a>
            + crate::traits::get_code_occurence::GetCodeOccurenceLifetimeWithSerializeDeserialize<'a>,
    ConfigGeneric: crate::traits::fields::GetSourcePlaceType
        + crate::traits::fields::GetTimezone
        + crate::traits::get_server_address::GetServerAddress,
{
    fn origin_to_string_with_config_lifetime_with_serialize_deserialize(&self, config: &ConfigGeneric) -> String {
        crate::traits::error_logs_logic::helpers::source_and_code_occurence_formatter(
            self.source_to_string_without_config_lifetime(),
            self.get_code_occurence_lifetime_with_serialize_deserialize().to_string_with_config_lifetime(config),
        )
    }
}
