...
impl UpdateParameters {
    ...
    DynArcGetConfigGetPostgresPoolSendSync,
        ...
        let query_string = {
            ...
            format!
            ("update {} as t set name = data.name, color = data.color from (select * from unnest($1,$2,$3)) as data(id, name, color) where t.id = data.id returning data.id",
            ROUTE_NAME,)
        ...
        let binded_query = {
            ...
            let(id_vec, name_vec, color_vec) = self.payload.into_iter().fold((
