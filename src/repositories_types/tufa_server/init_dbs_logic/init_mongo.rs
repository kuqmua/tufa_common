...
pub async fn init_mongo<'a>(
    ...
    match crate::repositories_types::tufa_server::mongo_integration::mongo_client_options_parse::mongo_client_options_parse(config).await {
        ...
        Ok(client_options) => match crate::repositories_types::tufa_server::mongo_integration::mongo_client_with_options::mongo_client_with_options(client_options) {
            Err(e) => Err(Box::new(
            ...
            Ok(client) => {
                ...
                if let Err(e) =
                    crate::repositories_types::tufa_server::mongo_integration::mongo_insert_many::mongo_insert_many(providers_json_local_data_hashmap, db, config).await
                {
                    return Err(Box::new(
